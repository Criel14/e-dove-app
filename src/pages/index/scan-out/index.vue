...
import { parcelOut } from '@/api/parcel/index.js'
...
async function handleScanOut() {
  ...
  try {
    ...
    submitError.value = error?.message || '出库失败，请重试'
    await uni.showToast({
      title: submitError.value,
      icon: 'none',
    })
  ...
  <view class="h-full bg-gray-50 p-[32rpx]">
    <view class="mt-[24rpx] rounded-[20rpx] bg-white p-[32rpx]">
      ...
      <view class="mt-[16rpx] text-[26rpx] leading-[40rpx] text-[#6B7280]">
        点击下方按钮后会打开摄像头扫描运单号条形码，识别成功后自动调用出库接口。
      </view>
      ...
      <button
        ...
        @click="handleScanOut"
      >
        {{ isScanning ? '扫码中...' : (isSubmitting ? '出库中...' : '扫码出库') }}
      </button>
    </view>
  </view>
</template>
